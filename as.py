...
SCRAPEDO_API_KEY = os.environ.get('SCRAPEDO_API_KEY', 'ca85f3ff8abb4061836f33cae33f8777d4627c554af')
...
async def check_gmail_exists(email: str) -> bool:
    """
    Validates email deliverability using Scrape.do API.
    Checks existence of Google/Gmail address.
    """
    email = email.strip().lower()
    if not email.endswith("@gmail.com"):
        return False
        
    target_url = urllib.parse.quote(f"https://accounts.google.com/InputEmail?Email={email}")
    url = f"http://api.scrape.do/?url={target_url}&token={SCRAPEDO_API_KEY}"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=15) as response:
                if response.status == 200:
                    ...
                    # If Google reports "Couldn't find your Google Account"
                    if "Couldn't find your Google Account" in text or "identifierNotFound" in text:
                        print(f"Scrape.do verification for {email}: Invalid")
                        return False
                    print(f"Scrape.do verification for {email}: Valid")
                    return True
    except Exception as e:
        print(f"Error checking email via Scrape.do API: {e}")
        return True
    return True
...
async def handle_sell(message: Message, state: FSMContext):
    ...
    # Real-time verification using Scrape.do API
    if email and "@gmail.com" in email:
        checking_msg = await message.answer("🔍 <b>Verifying Gmail account via Scrape.do...</b>", parse_mode=ParseMode.HTML)
...
async def handle_task_submission(message: Message, state: FSMContext):
    ...
    if email and "@gmail.com" in email:
        ...
        if not exists:
            await message.answer(
                f'<tg-emoji emoji-id="5447644880824181073">⚠️</tg-emoji> <b>Gmail Account Not Found!</b>\n\n'
                f'The account <code>{email}</code> does not exist or is invalid.\n\n'
